  ...
  module Auth
    class RegistrationsController < BaseController
      ...
      def handle_password_registration(user)
        target_path, target_subdomain, target_host = registration_redirect_destination(user)
        if Searls::Auth.config.email_verification_mode != :none
          EmailsVerification.new.email(user: user, redirect_path: target_path, redirect_subdomain: target_subdomain, redirect_host: target_host)
        ...
        if Searls::Auth.config.email_verification_mode == :required
          ...
          redirect_to searls_auth.pending_email_verification_path({email: user.email, redirect_path: target_path, redirect_subdomain: target_subdomain, redirect_host: target_host}.compact_blank)
        else
      ...
      def handle_email_registration(user)
        ...
        target_path, target_subdomain, target_host = registration_redirect_destination(user)
        enqueue_login_verification_email(user, target_path:, target_subdomain:, target_host:)
        flash[:notice] = Searls::Auth.config.resolve(:flash_notice_after_registration, user, params)
      ...
      def registration_redirect_destination(user)
        if redirect_params_supplied?
          [params[:redirect_path], params[:redirect_subdomain], params[:redirect_host]]
        else
      ...
      def redirect_params_supplied?
        params[:redirect_path].present? || params[:redirect_subdomain].present? || params[:redirect_host].present?
      end

      def enqueue_login_verification_email(user, target_path:, target_subdomain:, target_host:)
        email_otp = nil
